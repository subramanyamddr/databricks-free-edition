...
table_metadata = ast.literal_eval(dbutils.widgets.get("table_metadata"))
...
try:
    if source_system == "sqlserver":
        ...
        secret_json = dbutils.secrets.get(
            scope="banking-scope-2",
            key="sqlserver-connection-json-2"
        )

        config = json.loads(secret_json)

        jdbc_url = f"jdbc:sqlserver://{config['host']}:{config['port']};database={config['database']};encrypt=true;trustServerCertificate=false;"
